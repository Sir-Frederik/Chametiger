...
def make_tray_icon() -> Image.Image:
    """Genera un'icona colorata semplice per il tray."""
    size = 64
    img  = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    # Gradiente simulato con cerchio
    draw.ellipse([4, 4, 60, 60], fill=(70, 130, 200))
    draw.ellipse([16, 16, 48, 48], fill=(200, 230, 255))
    return img
...
class ChamerTigerTray:
    ...
    def _build_menu(self):
        autostart_label = lambda item: (
            "✓ Avvio con Windows" if is_autostart_enabled()
